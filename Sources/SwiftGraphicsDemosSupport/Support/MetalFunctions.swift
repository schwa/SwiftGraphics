...
func step<T>(_ edge: T, _ x: T) -> T where T: FloatingPoint {
    x < edge ? 0 : 1
}
...
func copysign<T>(x: T, y: T) -> T where T: FloatingPoint {
    y >= .zero ? x : -abs(x)
}

// x – y if x > y; +0 if x <= y.
func fdim<T>(x: T, y: T) -> T where T: FloatingPoint {
    x > y ? x - y : 0
}
