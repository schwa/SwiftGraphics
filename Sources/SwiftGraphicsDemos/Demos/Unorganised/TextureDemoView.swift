...
struct TextureView: View {
    ...
    var body: some View {
        MetalView { device, configuration in
            ...
            let library = try! device.makeDefaultLibrary(bundle: .swiftGraphicsDemosShaders)
            ...
            try renderState.commandQueue.withCommandBuffer(drawable: currentDrawable) { commandBuffer in
                ...
                try commandBuffer.withRenderCommandEncoder(descriptor: renderPassDescriptor) { renderCommandEncoder in
                    ...
                    let material = SwiftGraphicsDemosShaders.UnlitMaterial(color: [1, 0, 0, 1], textureIndex: 0)
