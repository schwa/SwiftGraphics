...
struct CSGDemoView_: View {
    ...
    init() {
        ...
        do {
            let a = CSGNode(polygons: a.polygons)
            let b = CSGNode(polygons: b.polygons)
            a.clip(to: b)
            node = a
        }
...
extension GraphicsContext {
    func draw(csg: CSG<SIMD3<Float>>, transform: CGAffineTransform = .identity, with shading: Shading) {
    ...
    func draw(node: CSGNode<SimpleVertex>, transform: CGAffineTransform = .identity) {
