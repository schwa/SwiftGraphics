...
struct CustomStrokeView: View {
    ...
    let strokeStyle = StrokeStyle(lineWidth: 120, lineCap: .butt, lineJoin: .miter, miterLimit: 199990)
    ...
    var body: some View {
        Canvas { context, size in
            ...
            let points = [60.0, -60.0].map { offset in
                return [segments.first!.parallel(offset: offset).start] + segments.windows(ofCount: 2).map { segments in
                    let segments = Array(segments)
                    let first = segments[0].parallel(offset: offset)
                    let second = segments[1].parallel(offset: offset)
                    if case let .point(point) = Line.intersection(first.line, second.line) {
                        return point
                    }
                    else {
                        fatalError()
