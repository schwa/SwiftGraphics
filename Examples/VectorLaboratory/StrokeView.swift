...
struct CustomStrokeView: View {
    ...
    var body: some View {
        Canvas { context, size in
            ...
            for window in AnySequence({ zip(self.segments, widths).peekingWindow() }) {
                ...
                if let previousWidth, let previousLeft = previousSegment?.parallel(offset: -previousWidth / 2.0), case let .point(point) = LineSegment.intersection(previousLeft, currentLeftSegment) {
                    context.drawDot(at: point)
                    currentLeftSegment.start = point
                }
                if let previousWidth, let previousRight = previousSegment?.parallel(offset: +previousWidth / 2.0), case let .point(point) = LineSegment.intersection(previousRight, currentRightSegment) {
                    context.drawDot(at: point)
                    currentRightSegment.start = point
                }
                if let nextWidth, let nextLeft = nextSegment?.parallel(offset: -nextWidth / 2.0), case let .point(point) = LineSegment.intersection(currentLeftSegment, nextLeft) {
                    context.drawDot(at: point)
                    currentLeftSegment.end = point
                }
                if let nextWidth, let nextRight = nextSegment?.parallel(offset: +nextWidth / 2.0), case let .point(point) = LineSegment.intersection(currentRightSegment, nextRight) {
                    context.drawDot(at: point)
